...
export function Button(props: Button.Props) {
  ...
  const button = (
    <button
      ref={ref}
      style={props.style}
      onClick={props.disabled || props.loading ? undefined : (props.onClick as never)}
  ...
  export const useColors = ({
    ...
    useMemo(
      () =>
        classes(
          {
            brand: "items-center justify-center bg-brand-1000 hover:bg-brand-1100",

            secondary: "bg-brand-200 bg-brand-200 rounded hover:bg-brand-25",

            ternary:
              "bg-brand-100 hover:bg-brand-25 border border-solid border-brand-200 box-border text-sm",
            ...
            whitesmoke:
              "bg-whitesmoke hover:bg-brand-25 hover:text-white active:bg-brand-25",

            white: "bg-white hover:bg-brand-25 hover:text-white active:bg-brand-25",
            ...
            outline:
              "shadow-none dark:text-white text-black border border-transparent hover:border-zinc-700 focus:border-brand-500 active:bg-brand-500 active:text-white active:border-brand-500 hover:bg-brand-25",
    ...
    return useMemo(() => {
      const render = (icon: typeof iconRight, position: "left" | "right") => {
        const className = classes(
          "opacity-100 duration-100 group-hover:opacity-100 shrink",
          size === "sm" && "h-5 w-5",
        ...
        if (typeof icon === "function") return icon({className, strokeWidth: 2.2})
  ...
  export const useBadges = ({badgeRight, size, fullWidth}: Props) =>
    useMemo(() => {
      const render = (badge: typeof badgeRight, position: "right") => {
        const className = classes(position === "right" && !fullWidth && "ml-1 -mr-1")
