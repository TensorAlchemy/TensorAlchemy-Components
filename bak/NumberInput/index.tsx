...
export function NumberInput(rawProps: Input.Props) {
  ...
  const onBlur = useCallback(() => {
    ...
    if (parsed) {
      onValueChange(`${clamp(props.min ?? -Infinity, parsed, props.max ?? Infinity)}`)
    } else {
  ...
  return (
    ...
    >
      {props.label && (
        <label className="opacity-muted text-sm font-inter flex flex-row items-center gap-1">
          {props.label}
          {props.tooltip && (
            <Theme.Tooltip
              delay={750}
              ...
              openMechanism={isMobileDevice ? "click" : "hover"}
            >
              <img
                className="w-full relative h-5 overflow-hidden shrink-0"
                alt=""
                src="question.svg"
              />
            </Theme.Tooltip>
      ...
      >
        ...
        <div
          className={classes(
            "flex flex-row rounded pl-1 outline outline-1 outline-transparent duration-150 bg-brand-50",
        ...
        >
          ...
          <input
            className={classes(
              "rounded bg-brand-50 py-0.5 text-black outline-none placeholder:text-black/50 dark:text-white dark:placeholder:text-white/20",
              props.disabled && "cursor-not-allowed",
              props.fullWidth ? "w-full" : "w-[290px]",
              props.percentage && "w-12 text-right",
              "selection:bg-brand-1000",
            )}
            value={
...
export namespace Input {
  export type Props = Styleable & {
    ...
    tooltip?: React.DetailedHTMLProps<
  ...
  export const useParseNumber = ({number, min, max, step, clamping}: Props) =>
    useCallback(
      (value: string | number) => {
        ...
        return clamping ? clamp(min ?? -Infinity, fixed, max ?? Infinity) : fixed
      },
    ...
    return {
      icon: useMemo(() => {
        ...
        if (typeof icon === "string") {
          ...
          return (
            <h1
              className="opacity-muted -ml-1 flex h-7 w-7 cursor-ew-resize select-none"
      ...
      iconRight: useMemo(() => {
        ...
        const className = classes("opacity-60 h-7 w-7 p-1 select-none shrink-0")
