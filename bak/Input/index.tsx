...
function AutosizeTextArea({
  ...
  return (
    <textarea
      ref={textareaRef}
      className={classes(
        "dark:placeholder:text-muted-white w-full bg-brand-50 flex-1 w-full relative leading-[24px] font-inter text-white text-left [display:-webkit-inline-box] overflow-hidden text-ellipsis [-webkit-line-clamp:4] [-webkit-box-orient:vertical]",
        size === "sm"
          ? "h-12 px-2 py-1 text-sm"
          : size === "lg"
            ? "h-12 text-base"
            : "h-10 px-2 py-1 text-base",

        props.disabled && `cursor-not-allowed ${!props.transparent && "bg-black/10"}`,
  ...
  return (
    <div className={classes("relative", fullWidth && "w-full", className)}>
      ...
      <input
        ref={inputRef}
        className={classes(
          "dark:placeholder:text-muted-white h-full w-full rounded bg-brand-50 text-black shadow-sm focus:border-transparent focus:outline-none dark:border-none dark:text-white",
          sizing,
